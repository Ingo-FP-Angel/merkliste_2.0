...
suspend fun fetchAvailableMedias(user: String, pass: String, location: String, mediatype: String): Array<Media> {
    val backendBaseUrl = if (process.env.NODE_ENV == "production") "/api" else "http://localhost:8080/api"
    val responsePromise = window.fetch("$backendBaseUrl/media?location=$location&mediatype=$mediatype",
            object: RequestInit {
